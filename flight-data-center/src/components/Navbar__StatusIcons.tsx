import React, {useState} from "react";
import {TelemetryData} from "../Entity/TelemetryData";

const getLastBatteryCharge = (data: TelemetryData): number | null => {
    if (data.battery.length === 0) {
        return null;
    }
    return data.battery[data.battery.length - 1];
};

const Navbar__StatusIcons = (props: {drone: any}) => {
    let drone = props.drone;
    let batteryCharge: number | null = getLastBatteryCharge(drone.telemetryData);
    const [batteryProcentage, setBatteryProcentage] = useState<number | null>(batteryCharge);

    return (
        <div className="flex items-center justify-center space-x-4">
            <div className={'flex space-x-2 items-center'}>
                {
                    (batteryProcentage && batteryProcentage > 65) ?
                        <div className={'p-1 rounded-md bg-green-400 text-green-800'}>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M21 10.5h.375c.621 0 1.125.504 1.125 1.125v2.25c0 .621-.504 1.125-1.125 1.125H21M4.5 10.5H18V15H4.5v-4.5ZM3.75 18h15A2.25 2.25 0 0 0 21 15.75v-6a2.25 2.25 0 0 0-2.25-2.25h-15A2.25 2.25 0 0 0 1.5 9.75v6A2.25 2.25 0 0 0 3.75 18Z" />
                            </svg>
                        </div>
                        : ''
                }
                {
                    (batteryProcentage && batteryProcentage <= 65 && batteryProcentage > 30) ?
                        <div className={'p-1 rounded-md bg-orange-400 text-orange-800'}>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M21 10.5h.375c.621 0 1.125.504 1.125 1.125v2.25c0 .621-.504 1.125-1.125 1.125H21M4.5 10.5h6.75V15H4.5v-4.5ZM3.75 18h15A2.25 2.25 0 0 0 21 15.75v-6a2.25 2.25 0 0 0-2.25-2.25h-15A2.25 2.25 0 0 0 1.5 9.75v6A2.25 2.25 0 0 0 3.75 18Z" />
                            </svg>
                        </div>
                        : ''
                }
                {
                    (batteryProcentage && batteryProcentage <= 30) ?
                        <div className={'p-1 rounded-md bg-red-400 text-red-800'}>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M21 10.5h.375c.621 0 1.125.504 1.125 1.125v2.25c0 .621-.504 1.125-1.125 1.125H21M3.75 18h15A2.25 2.25 0 0 0 21 15.75v-6a2.25 2.25 0 0 0-2.25-2.25h-15A2.25 2.25 0 0 0 1.5 9.75v6A2.25 2.25 0 0 0 3.75 18Z" />
                            </svg>
                        </div>
                        : ''
                }
                <p>{batteryProcentage}%</p>
            </div>


        </div>
    );
}

export default Navbar__StatusIcons;